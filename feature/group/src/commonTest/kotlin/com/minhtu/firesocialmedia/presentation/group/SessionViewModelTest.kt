...
private class FakeGroupSessionUserRepository : UserRepository {
...
class SessionViewModelTest {
    ...
    fun `init loads current user when uid is available`() = runTest(dispatcher) {
        val userRepo = FakeGroupSessionUserRepository().apply {
            currentUserUid = "u1"
    ...
    fun `init leaves currentUser null when no uid`() = runTest(dispatcher) {
        val userRepo = FakeGroupSessionUserRepository().apply { currentUserUid = null }
        val vm = SessionViewModel(GetUserUseCase(userRepo), GetCurrentUserUidUseCase(userRepo))
        advanceUntilIdle()
    ...
    @Test
    fun `findUserById returns null when user not found`() = runTest(dispatcher) {
        val userRepo = FakeGroupSessionUserRepository()
        val vm = SessionViewModel(GetUserUseCase(userRepo), GetCurrentUserUidUseCase(userRepo))
        advanceUntilIdle()
