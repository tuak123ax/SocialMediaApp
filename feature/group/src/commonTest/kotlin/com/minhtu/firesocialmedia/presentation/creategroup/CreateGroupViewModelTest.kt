...
private class FakeCreateGroupRepo : GroupRepository {
    ...
    override suspend fun getGroupConfigs(userId: String, groupId: String): GroupConfigs = GroupConfigs()
...
class CreateGroupViewModelTest {
    ...
    fun `createGroup success updates createGroupState and adds to user groups`() = runTest {
        ...
        assertTrue(user.groups.contains(created!!.id))
    }
    ...
    @Test
    fun `createGroup failure sets createGroupState to empty GroupInstance and removes from user groups`() = runTest {
        val dispatcher = StandardTestDispatcher(testScheduler)
