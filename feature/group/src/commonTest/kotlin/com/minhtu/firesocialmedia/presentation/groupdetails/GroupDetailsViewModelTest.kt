...
private class FakeDetailsNewsRepo : GroupNewsRepository {
...
private fun makeVm(
    repo: FakeDetailsGroupRepo,
    newsRepo: FakeDetailsNewsRepo,
    ...
    deletePollUseCase = DeletePollUseCase(newsRepo),
...
class GroupDetailsViewModelTest {
    ...
    fun `fetchGroupInfo updates fetchGroupInfoState`() = runTest {
        ...
        val vm = makeVm(repo, FakeDetailsNewsRepo(), dispatcher)
    ...
    fun `leaveGroup with multiple members calls leaveGroup use case`() = runTest {
        ...
        val vm = makeVm(repo, FakeDetailsNewsRepo(), dispatcher)
        val user = UserInstance(uid = "u1").apply { groups.add("g1") }
    ...
    fun `leaveGroup as sole member calls leaveAndDeleteGroup use case`() = runTest {
        ...
        val vm = makeVm(repo, FakeDetailsNewsRepo(), dispatcher)
        val user = UserInstance(uid = "u1").apply { groups.add("g2") }
    ...
    @Test
    fun `init populates visibleImages with first 30 items`() {
        val dispatcher = StandardTestDispatcher()
        val vm = makeVm(FakeDetailsGroupRepo(), FakeDetailsNewsRepo(), dispatcher)
    ...
    @Test
    fun `loadMore appends next page of visibleImages`() {
        val dispatcher = StandardTestDispatcher()
        val vm = makeVm(FakeDetailsGroupRepo(), FakeDetailsNewsRepo(), dispatcher)
        val posts = (1..50).map { NewsInstance(id = "n$it", image = "img$it") }
        vm.init(posts)
    ...
    @Test
    fun `onPostsUpdated inserts new post at the top`() {
        val dispatcher = StandardTestDispatcher()
        val vm = makeVm(FakeDetailsGroupRepo(), FakeDetailsNewsRepo(), dispatcher)
    ...
    @Test
    fun `onPostsUpdated with same size does not modify visibleImages`() {
        val dispatcher = StandardTestDispatcher()
        val vm = makeVm(FakeDetailsGroupRepo(), FakeDetailsNewsRepo(), dispatcher)
