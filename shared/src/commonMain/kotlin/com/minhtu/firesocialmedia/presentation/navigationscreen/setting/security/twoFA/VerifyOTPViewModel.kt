...
class VerifyOTPViewModel(
    ...
    fun enableOTP(currentUser : UserInstance,
                  secret : String,
                  otpToVerify: String) {
        viewModelScope.launch(ioDispatcher) {
            _verifyOTPResult.value = enable2FAUseCase.invoke(
                currentUser,
                secret,
                otpToVerify
            )
    ...
    fun verifyOTP(currentUser : UserInstance,
                  otpToVerify: String) {
        viewModelScope.launch(ioDispatcher) {
            _verifyOTPResult.value = verify2FAUseCase.invoke(
                currentUser,
                otpToVerify
            )
        }
