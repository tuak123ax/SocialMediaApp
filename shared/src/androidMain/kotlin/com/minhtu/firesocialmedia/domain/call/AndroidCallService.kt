...
class AndroidAudioCallService(
    ...
    override suspend fun startVideoCall(
        onStartVideoCall: (videoTrack: WebRTCVideoTrack) -> Unit
    ) {
        if (hasStarted) {
    ...
    override suspend fun releaseResources() {
        try {
            ...
            logMessage("CallForegroundService") { "Released local audio track success" }
            // Video
            releaseVideoCallResource()
            logMessage("CallForegroundService") { "Released video call success" }
            ...
            logMessage("CallForegroundService") { "Released eglBase success" }

            // Tracer
            PeerConnectionFactory.shutdownInternalTracer()
            logMessage("CallForegroundService") { "Released tracer success" }
    ...
    /**
     * This function is used to release all resources related to video call.
     * */
    suspend fun releaseVideoCallResource() {
        ...
        videoCapturer?.let { capturer ->
            withTimeoutOrNull(2000) {
                try {
                    ...
                    logMessage("CallForegroundService") { "Error stopping video capturer: ${e.message}" }
                } finally {
